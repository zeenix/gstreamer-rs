...
pub trait ElementExtManual {
    ...
    fn query_convert(&self, src_val: ::FormatValue, dest_format: ::Format)
        -> Option<::FormatValue>;
    ...
    fn seek(
        ...
        start: ::FormatValue,
        stop_type: ::SeekType,
        stop: Option<::FormatValue>,
    ) -> Result<(), glib::error::BoolError>;
    fn seek_simple(
        &self,
        seek_flags: ::SeekFlags,
        seek_pos: ::FormatValue,
    ) -> Result<(), glib::error::BoolError>;
}
...
impl<O: IsA<Element>> ElementExtManual for O {
    ...
    fn seek(
        ...
        if let Some(stop) = stop {
            if stop.to_format() != start.to_format() {
                return Err(glib::error::BoolError(
                    "Incompatible start and stop formats",
                ));
            }
        }
        ...
        unsafe {
            glib::error::BoolError::from_glib(
                ffi::gst_element_seek(
                    ...
                    start.to_value(),
                    stop_type.to_glib(),
                    stop.map(|v| v.to_value()).unwrap_or(-1),
                ),
                "Failed to seek",
            )
        }
